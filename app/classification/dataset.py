...
def ensure_local_data():
    """
    Check whether the classification dataset exists locally.

    - If it exists and is not empty -> do nothing
    - Otherwise -> download it from GCS (see GCS_CLASSIFICATION_DATA in params)

    Note: the download destination is CLASSIFICATION_DATA_DIR's PARENT on
    purpose. `gcloud storage cp --recursive` copies the SOURCE FOLDER ITSELF
    into the destination, so pointing it at data/ is what lands the images at
    data/lego-dataset-classification rather than one level too deep.
    """
    ...
    if data_exists:
        print(f"✅ Dataset already present: {CLASSIFICATION_DATA_DIR}")
        return

    print(
        f"⬇️  Dataset not found locally -- downloading from {GCS_CLASSIFICATION_DATA}")
    ...
    print(f"✅ Dataset downloaded successfully: {CLASSIFICATION_DATA_DIR}")
...
def select_classes(num_classes: int = NUM_CLASSES) -> list[str]:
    ...
    print(
        f"✅ Selected {len(selected)} classes of {len(counts)} available "
        f"(ranked by real-photo count)"
    )
    return selected
...
def build_dataframe(classes: list[str] | None = None) -> pd.DataFrame:
    ...
    n_photos = int((dataframe["source"] == "photos").sum())
    n_renders = int((dataframe["source"] == "renders").sum())
    print(
        f"✅ Indexed {len(dataframe):,} images "
        f"({n_photos:,} photos / {n_renders:,} renders) "
        f"across {dataframe['label'].nunique()} classes"
    )
    return dataframe
...
def encode_labels(dataframe: pd.DataFrame, persist: bool = True):
    ...
    print(
        f"✅ Labels encoded: {len(encoder.classes_)} part IDs -> indices 0..{len(encoder.classes_) - 1}")
...
def cap_renders(
    ...
    n_kept_renders = len(capped) - len(photos)
    print(
        f"✅ Renders capped in training split: kept {n_kept_renders:,} of "
        f"{len(renders):,} renders (all {len(photos):,} photos kept, "
        f"ratio={ratio:.2f})"
    )
    return capped
...
def split_dataframe(
    ...
    print(
        f"✅ Stratified split: {len(train_df):,} train / "
...
def get_datasets(
    ...
    """
    # Fetch the dataset first if this machine doesn't have it yet. Cheap
    # no-op when the images are already on disk.
    ensure_local_data()
